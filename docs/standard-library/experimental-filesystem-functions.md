...
- Otherwise, if `!exists(t) && options & copy_options::copy_symlinks`, then `copy_symlink(from, to)`.
...
- Otherwise, if `opts & copy_options::create_symlinks`, then `create_symlink(from, to)`.

- Otherwise, if `opts & copy_options::create_hard_links`, then `create_hard_link(from, to)`.

- Otherwise, if `is_directory(t)`, then `copy_file(from, to / from.filename(), opts)`.
