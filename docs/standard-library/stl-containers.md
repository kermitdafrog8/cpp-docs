...
A `map`, sometimes referred to as a dictionary, consists of a key/value pair. The key determines the sequence order and identifies its associated value. For example, a `map` might contain keys that represent every unique word in a text and corresponding values that represent the number of times that each word appears in the text. Unlike `map`, `unordered_map` doesn't sort its elements. For more information, see [`map` Class](../standard-library/map-class.md) and [`unordered_map` Class](../standard-library/unordered-map-class.md).

A `set` is an ascending container of unique elements - the value is also the key. Unlike `set`, `unordered_set` doesn't sort its elements. For more information, see [`set` Class](../standard-library/set-class.md) and [`unordered_set` Class](../standard-library/unordered-set-class.md).
